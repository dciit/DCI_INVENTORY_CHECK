...
import { API_TAG_RECORD_AUDITOR, API_TEG_SELECT_QR } from '@/service/gentag.service';
...
function AuditorScanData() {
    ...
    const handleInputData = async () => {
        ...
        const clonedArray = selectedInfo.map(item => ({
            ...item,
            auditorQty: valuesData.qty ? Number(valuesData.qty) : undefined,
            ...
            {selectedInfo && selectedInfo.length > 0 ? (
                selectedInfo.map((item, idx: number) => (
                    <div className="flex flex-col  items-center justify-center gap-3 w-[50%] mt-2" key={idx}> {/* Add a key prop for list rendering */}
                        <div className="container w-[60%] rounded-xl border border-black shadow-lg p-10 text-center bg-[#FBFBFB]">
                            <div className="flex flex-col gap-3">
                                <div className="flex flex-row justify-end">
                                    <div className="w-44 p-3 border border-black rounded-md bg-[#FFF5D7] text-lg text-black font-semibold text-start">
                                ...
                                <div className="flex flex-row mt-3 gap-5">
                                    ...
                                    <div className="p-3 border border-black rounded-md text-xl text-black font-semibold text-start w-full">{item.ym}</div>
                                </div>
                                <div className="flex flex-row mt-3 gap-5">
                                ...
                                <div className="flex flex-row gap-5 mt-3">
                                    ...
                                    <div className="p-3 border border-black rounded-md text-xl text-black font-semibold text-start w-3/4">{item.partNo}</div>
